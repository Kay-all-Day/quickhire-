...
$avgFeedbackRating = 0;
$issueCategories = ['service_issue', 'provider_complaint', 'payment_issue'];
try {
    $stmt = $pdo->query("
    ...
    foreach ($allFeedback as $fb) {
        if (in_array($fb['category'], $issueCategories)) {
            $issuesList[] = $fb;
        } else {
            $generalList[] = $fb;
        }
        if (!$fb['is_read']) $pendingFeedbackCount++;
    }

    $stmt = $pdo->query("SELECT ROUND(AVG(rating), 1) as avg FROM platform_feedback WHERE category NOT IN ('service_issue','provider_complaint','payment_issue')");
...
<body>
  ...
  <div class="admin-layout">
    ...
    <main class="admin-main">
      ...
      <div id="feedback-admin" class="admin-panel">
        <h2 class="dash-panel-title">Feedback & Issues</h2>

        <!-- Issues Reported -->
        <h3 class="dash-panel-sub" style="font-family:'Sora',sans-serif;font-size:1.05rem;font-weight:800;color:var(--bark);margin-bottom:14px;">Issues Reported</h3>
        <?php if (empty($issuesList)): ?>
          <div class="profile-section" style="margin-bottom:28px;"><p>No issues reported yet.</p></div>
        <?php else: ?>
        <div class="profile-section" style="margin-bottom:32px;">
          <?php foreach ($issuesList as $fb): ?>
          <div style="background:var(--card-bg);border:1.5px solid rgba(249,115,22,0.3);border-radius:12px;padding:20px;margin-bottom:14px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
              <div>
                <span style="font-weight:700;font-size:0.92rem;"><?= htmlspecialchars($fb['full_name']) ?></span>
                <span style="font-size:0.72rem;padding:2px 8px;border-radius:10px;margin-left:8px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;background:rgba(249,115,22,0.1);color:#c2410c;"><?= ucfirst(str_replace('_', ' ', $fb['category'])) ?></span>
              </div>
        ...
        <!-- General Feedback -->
        <h3 class="dash-panel-sub" style="font-family:'Sora',sans-serif;font-size:1.05rem;font-weight:800;color:var(--bark);margin-bottom:6px;">General Feedback</h3>
        <p style="font-size:0.85rem;color:var(--sand);margin-bottom:14px;">Average rating: <strong style="color:var(--ember);">★ <?= $avgFeedbackRating ?: 'N/A' ?></strong></p>
        <?php if (empty($generalList)): ?>
          <div class="profile-section"><p>No general feedback submitted yet.</p></div>
        <?php else: ?>
        <div class="profile-section">
          <?php foreach ($generalList as $fb):
            $fbStars = str_repeat('★', $fb['rating']) . str_repeat('☆', 5 - $fb['rating']);
          ?>
          <div style="background:var(--card-bg);border:1.5px solid var(--border);border-radius:12px;padding:20px;margin-bottom:14px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
              <div>
                <span style="font-weight:700;font-size:0.92rem;"><?= htmlspecialchars($fb['full_name']) ?></span>
                <span style="font-size:0.72rem;padding:2px 8px;border-radius:10px;margin-left:8px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;background:rgba(13,148,136,0.08);color:var(--ember);"><?= ucfirst(str_replace('_', ' ', $fb['category'])) ?></span>
              </div>
              <div style="display:flex;align-items:center;gap:10px;">
  ...
  <script>
    const PANEL_BADGE = {
      'feedback-admin':     'badge-feedback',
      'verification-admin': 'badge-verification',
      'featured-requests':  'badge-featured',
    };
    ...
    function showAdmin(id, el) {
      ...
      const badgeId = PANEL_BADGE[id];
      if (badgeId) { const b = document.getElementById(badgeId); if (b) b.remove(); }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      document.querySelectorAll('input[name="action"][value="mark_feedback_read"]').forEach(inp =>
        inp.closest('form').addEventListener('submit', () => decBadge('badge-feedback'))
      );
      ['approve_verification','reject_verification'].forEach(action =>
        document.querySelectorAll(`input[name="action"][value="${action}"]`).forEach(inp =>
          inp.closest('form').addEventListener('submit', () => decBadge('badge-verification'))
        )
      );
      document.querySelectorAll('input[name="action"][value="approve_featured"]').forEach(inp =>
        inp.closest('form').addEventListener('submit', () => decBadge('badge-featured'))
      );
    });
