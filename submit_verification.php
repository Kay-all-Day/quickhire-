...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ══════════ Smile ID auto-verification ══════════
    $providerName = getUserName();
    $smile = smileid_verify_id($id_type, $id_number, $providerName);
    ...
    ");
    $stmt->execute([
        $smile['status'], $smile['summary'], $smile['reference'],
        $smile['response'], $requestId,
    ]);
    ...
    $smileNote = '';
    if ($smile['status'] === 'verified') {
        $smileNote = ' Smile ID auto-verified ✓';
    } elseif ($smile['status'] === 'failed') {
        $smileNote = ' Smile ID flagged this request ⚠';
    }
    foreach ($admins as $admin) {
        createNotification($pdo, $admin['user_id'], 'verification', 'New Verification Request',
            $providerName . ' has submitted documents for verification.' . $smileNote, 'admin.php');
    }
