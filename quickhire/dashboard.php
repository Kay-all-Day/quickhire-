  ...
  <div class="dash-layout">
    ...
    <main class="dash-main">
      ...
      <div id="support" class="dash-panel">
        ...
        <div class="profile-section" style="margin-top:20px;">
          <h3>My QuickHire Ratings</h3>
          <?php foreach ($myRatings as $mf):
            $mfStars = str_repeat('★', $mf['rating']) . str_repeat('☆', 5 - $mf['rating']);
          ?>
          <div style="background:var(--cream);border:1px solid var(--border);border-radius:10px;padding:18px;margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
              <span style="color:var(--ember);font-size:0.9rem;"><?= $mfStars ?></span>
              <span style="font-size:0.75rem;color:var(--sand);"><?= date('j M Y', strtotime($mf['created_at'])) ?></span>
            </div>
            <?php if ($mf['message'] !== '(No comment)'): ?>
            <p style="font-size:0.88rem;color:var(--bark);line-height:1.6;"><?= htmlspecialchars($mf['message']) ?></p>
            <?php endif; ?>
            <p style="font-size:0.72rem;color:var(--sand);margin-top:6px;">✓ Submitted - Thank you for your feedback!</p>
          </div>
          <?php endforeach; ?>
        </div>
        <?php endif; ?>
